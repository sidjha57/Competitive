...
void solve() {
    ...
    ll rem = 1, n = factorial[s.length()],sum=0;
    for (auto x : freq) rem = (rem%mod * inverse_mod(factorial[x.second])%mod)%mod; 
    ll res = ((n%mod)*(rem%mod))%mod;
    ll i = 0;
    while (i <= freq.size()-2) {
        res = (res * inverse_mod(factorial[i])) % mod;
        sum = (sum%mod + res%mod)%mod;
        i++;
    }
    sum = (sum + freq.size())%mod;
