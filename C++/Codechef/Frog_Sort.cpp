...
if(count==1)return 0; else return 1;}} //at max 15812 times for 9 digit long number
 ...
 int main(){
    _cin// for fast I/O
     ...
     ll t,n,count,i,index,factor,step;
    
     cin>>t;
     while(t--){
      cin>>n;
      vl W(n),L(n),sortedW(n);
      f (i,n) cin >> W[i];
      f (i,n) cin >> L[i];
      count = 0;
      sortedW = W;
    ...
    for (i=0; i <n-1; i++) {
      if (pos[sortedW[i+1]] > pos[sortedW[i]]) 
      continue;
      else {
         factor= L[find(W.begin(), W.end(),sortedW[i+1]) - W.begin()]; 
         step =pos[sortedW[i]] - pos[sortedW[i+1]];
         step = step / factor +1;
         count += step;
         pos[sortedW[i+1]] += step*factor;
