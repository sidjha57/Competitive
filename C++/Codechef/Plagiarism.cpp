...
if(count==1)return 0; else return 1;}} //at max 15812 times for 9 digit long number
 ...
 int main(){
    _cin// for fast I/O
     ...
     ll t,n,m,k,a,b,c,i,j;
     cin>>t;
     while(t--){
      cin >>n>>m>>k;
      map <ll,ll> frequency_table;
      for (i=0;i<k;i++) {
          cin >> a;
          frequency_table[a]++;
      }
      ll user_to_be_disqualified=0;
      ll arr[k]; // array to maintain which user are supposed to be disqualified
      j=0;
      for (auto x : frequency_table) {
          if (x.first <= n && x.second > 1) { 
              user_to_be_disqualified++;
              arr[j++]=x.first;
          }
      }
      cout << user_to_be_disqualified << " ";

      for (i=0;i<user_to_be_disqualified;i++) cout << arr[i] << " ";
      cout << "\n";
