...
if(count==1)return 0; else return 1;}} //at max 15812 times for 9 digit long number
 ...
 int main(){
    _cin// for fast I/O
     ...
     ll n,x,i,money;
     cin >> n >>x;
     vl cost(n+1),pages(n+1);
     for (i=1;i<n+1;i++) cin >> cost[i];
     for (i=1;i<n+1;i++) cin >> pages[i];
     ll dp[n+1][x+1];
     ...
     for (i =0;i<=n;i++) {
         for (money=0;money<=x; money++) {
             if (money == 0 || i == 0) dp[i][money] = 0;
                 ...
                 ll op1 =(i == 1) ? 0 :dp[i-1][money];
                 ll op2 =(money < cost[i]) ? 0 : pages[i] + dp[i-1][money - cost[i]];
                 dp[i][money] = max <ll> (op1, op2);
             }
         }
     }
