...
int getMax (multiset<int>& ms) {
    ...
    bool check(int k, vector<int>& chargeTimes, vector<int>& runningCosts, long long budget ) {
        long long sum = 0, n = chargeTimes.size();
        multiset<int> ms;
        for (int i=0; i<k; i++) {
        ...
        for (int i=0,j=k; j<n; i++,j++) {
            ...
            sum += k*(runningCosts[j] - runningCosts[i]);
...
void solve() {
    vector<int> charge = {11,12,19}, running = {10,8,7};
    long long budget = 19;
    cout << maximumRobots(charge,running,budget);
}
